...
| `ZSH_DISABLE_COMPFIX=true` で compaudit スキップ | ~70 ms | 低(insecure dir 警告が出なくなるだけ) |
| glob 深さ制限 + ソート | ~90 ms | 低(ファイル集合・順序同一を担保) |
